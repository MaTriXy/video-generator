...
<overview>
Generate SVG path `d` attributes using the `mcp__video_gen_tools__svg_path` tool.
</overview>
...
<merging-paths>
Use `mcp__video_gen_tools__merge_paths` tool to combine multiple path strings into a single continuous path.
